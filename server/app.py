...
from tensorflow.keras.layers import (
    ...
    Bidirectional, Dropout, SpatialDropout1D,
...
class SelfAttention(Layer):
    def build(self, input_shape):
        ...
        self.W = self.add_weight(shape=(feat_dim, feat_dim),
                                 initializer='glorot_uniform',
                                 trainable=True)
        self.b = self.add_weight(shape=(feat_dim,),
                                 initializer='zeros',
                                 trainable=True)
        self.u = self.add_weight(shape=(feat_dim, 1),
                                 initializer='glorot_uniform',
                                 trainable=True)
...
MODEL_PATH = os.path.join(ROOT_DIR, "deep_learning", "trained_models", "best_sentiment_model.pkl")
